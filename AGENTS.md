...
- **Sleep timer**: `Playback.sleep(minutes)` arms a `setTimeout` that disarms rotation, then calls `PlayerPower.displayOff()`, falling back to `stopForIdle()` when no display command is configured. `POST /api/player/sleep` with `minutes<=0` cancels; state surfaces in the system summary as `sleep:{active,deadline}`.
