...
- API 从 6 个端点收缩到 3 个。
- Settings 收缩成一张存储卡片。
- Settings 暴露 SMB share 内的媒体存放路径,避免默认写共享根目录。
...
- 不做域(domain)、mount options 的 UI;SMB 子目录只作为安全校验后的相对媒体路径。
- 不删 helper、`Bun.secrets`、sentinel 校验、mount 选项白名单、播放中禁切保护。
- 不写旧 config 迁移代码 —— storage 结构已经固定为 `{ mode, smb }`;Effect Schema 解码会忽略残留的旧 `connections` 字段。
...
"storage": {
  ...
  "smb": null               // null 或 { "server", "share", "username", "path?" }
}
```
...
- NAS 的内部连接名固定为 `"smb"`(keyring key + 挂载目录名)。
- `domain=""`、`mount_options=默认安全集` 全为内部常量,不进 config、不进 UI。
- `smb.path` 是共享内相对媒体目录。空字符串兼容共享根;新 UI 默认建议
  `pi-wallpaper-engine`。后端拒绝绝对路径、`..`、空 path segment、反斜杠、换行和 NUL。
...
PUT  /api/storage
  body: { mode: "local"|"mounted_share", smb: null | { server, share, username, path?, password? } }
...
StorageStatus {
  ...
  smb: { server, share, username, path: string, has_password: boolean } | null
...
1. 校验 `smb` 字段(`server`/`share`/`username` 非空;`path` 为安全相对路径);带 `password` 则存入 keyring。
2. 写 `smb` 到 config。
...
4. 仅改 `smb` 凭据不触发迁移;若此时 `mode=mounted_share`,重新挂载(卸旧挂新)。
   已有库且正在使用 SMB 时,禁止直接改 `server`/`share`/`path`;正确流程是先切回本机
   (数据搬回 SD),再改 SMB 位置并切过去。
...
```
拷贝全部目录 → 校验全部目录 → 持久化目标 mode → 删旧源
```

源只在校验通过并且目标 mode 已成功写入 config 后才删。这样即使 config 写入失败，
最多留下目标副本，不会出现"源已删但 mode 仍指向旧 root"。NAS→local 时，SMB 挂载
保持到远端旧源清理完成后再卸载。
...
export async function moveTree(opts: {
  from: string
  to: string
  onProgress?: (movedBytes: number) => void
  signal?: AbortSignal
}): Promise<void>
// 实现步骤:
//  1. from 不存在 → 直接返回。
//  2. mkdir -p to。
//  3. Bun.spawn(["rsync","-r","--partial","--size-only","--info=progress2", from+"/", to+"/"]);
//     按 \r 切流解析进度行首个数字(去逗号)= movedBytes,onProgress 上报(节流 ~1/s);
//     signal abort → kill 进程 → throw { code: "Cancelled" }。
//  4. 退出码 ≠ 0 → throw { code: "Copy" }。
//  5. 校验:Bun.spawn(["rsync","-rni","--size-only", from+"/", to+"/"])(dry-run、不带 --delete);
...
- `Migrate` Tag + `MigrateLive` 层;依赖 `Config`、`Logger`、`Mpv`、`Storage`。
- 持有当前任务 `Ref<MigrationProgress | null>` + 当前 fiber。
- `start(targetMode)`:
  ...
  6. 播放器若正在播放源 root 内文件 → fail `Busy`。
  7. 写任务 Ref(running);`Effect.forkDaemon` 跑任务体。
  8. 任务体:先对 `source`、`optimized` 依次 `copyTree`,再全部 `verifyTree`;
     onProgress → 更新 Ref。
  9. 成功提交:先 `Storage.finishMigration(targetMode)` 落盘新 mode,再 `removeTree`
     清理旧源;`NAS→local` 清理完成后 `Storage.applyMode("local")` 卸载 NAS。
  10. 失败 → 任务 state=failed + 友好文案;mode 不变。
...
- `mediaRootFor(mode)` —— 读取当前 SMB path 后算指定模式的媒体根(供 `Migrate`)。
- `saveSmb(input)` —— 只保存 SMB 配置/密码,不切换 mode。
- `applyMode(mode)` —— 无需迁移时的即时 mode 写入 + reconcile。
...
```
 Frontend Settings
   │ GET/PUT /api/storage · POST /cancel
   ▼
 routes/storage.ts ───────────────┐
   │ saveSmb/applyMode             │ start / cancel / status
   ▼                               ▼
 Storage service             Migrate service
  mode/smb/挂载                任务 Ref + 轮询进度
...
- 选 NAS:五个输入框 `地址` / `共享名` / `存放路径` / `用户名` / `密码`(已存密码时占位符显示"保持不变")+ `保存` 按钮。
- 一行状态:`✓ 可用` / `✗ <友好文案>`。
- 切换模式触发 `PUT /api/storage`;若返回 202(需迁移)→ 展示**迁移进度条**
  (`moved/total` + 百分比)+ `取消` 按钮,数据通过轮询 `GET /api/storage` 取得。
...
新增 `StorageStatus`(新形)/`SmbInput`/`getStorage`/`updateStorage`/`cancelMigration`。
`SystemSummary.storage` 改为新形。
...
**shared**
5. `src/schema/Config.ts` —— `StorageConfig` 改 `{ mode, smb }`;`SmbConfig` 含可选 `path`;
   删 `SmbConnectionConfig`/`StorageConnectionType`/`mount_base`/`mount_sentinel`/
...
8. `src/services/Config.ts` —— `RuntimeStorageConfig` 改 `{ mode, smb }`,`withStorageDefaults` 重写并补 `smb.path` 默认值。
9. `src/services/Storage.ts` —— 主改:单 `smb`、常量化挂载路径、接口调整、周期 reconcile fiber
   (`Layer.scoped` + `forkScoped`)、友好错误翻译。
10. `src/services/Migrate.ts`(新)
11. `src/routes/storage.ts` —— 收缩为 `GET /`、`PUT /`、`POST /cancel`。
...
15. `src/preflight.ts` —— storage 检查改读 `config.storage.smb`,挂载路径用常量。
...
**其他**
22. `config.example.json` —— `"storage": { "mode": "local", "smb": null }`。
...
- `mode` 只在迁移**校验通过后**翻;源只在目标 mode 成功落盘后删。
- 任何复制/校验/config 写入失败都保持原模式,原数据完好。
...
storage 功能已提交。代码回滚需同时处理 `config.storage` 新旧结构、`@pwe/migrate`
workspace 依赖和前端 Settings API。运行时一次失败的迁移仍由设计保证 mode 不变 + 源完好。
...
- 纯 rsync 薄封装(`estimateSize`、`copyTree`、`verifyTree`、`removeTree`、`moveTree`)。
  后端迁移用 copy/verify/remove 分阶段提交,`moveTree` 作为包级便利函数保留。
...
- `api.ts`:删 `StorageConnectionRecord`/`StorageConnectionInput`/连接 CRUD 函数;新增 `MigrationProgress`/`SmbRecord`/`SmbInput`/`StorageUpdate` 类型 + `getStorage`/`updateStorage`/`cancelMigration`;`SmbRecord` 包含 `path`。`SystemSummary.storage` 同步收缩。
- `pages/Settings.tsx`:Storage 部分整个收缩为一张卡片(segmented 本机 / 网络存储、5 字段 SMB 表单、状态行、迁移进度条 + 取消按钮);SWR `refreshInterval` 函数化,迁移中 1s、空闲 5s。删除连接列表 / 编辑器 / Connect / Disconnect / mount options / domain。窗口 `confirm` 在非空库切换时弹确认。
...
- `4321441 feat(pkg): add declarative SMB storage and migration`
- `cb2fc36 feat(pkg): add configurable SMB media path and English README`

仍建议在真 Pi + 真 SMB share 上做手动验收,因为实际 mount/reconnect/迁移速度无法用本地单测完全覆盖。
